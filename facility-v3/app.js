// THE FACILITY SIMULATION LOGIC -- V3 (CLOCK-DRIVEN RULESET)
// V3 removes the energy stat entirely. The working day IS the budget: every
// action costs hours, and the shift ends at 17:00. Same action economy as V2
// (which spent exactly 2 energy per hour), expressed in the natural unit.

const TOTAL_DAYS = 7;
const START_HOUR = 9;   // shift opens 09:00
const END_HOUR = 17;    // shift closes 17:00 -- nothing happens after this
const SHIFT_HOURS = END_HOUR - START_HOUR; // 8 workable hours per day
const FATIGUE_DAYS = 1; // 1 day of fatigue -- deaths already thin the roster
const MEET_HOURS = 1;   // meeting someone takes an hour
const SCAN_HOURS = 2;   // either indicator takes two hours

// The clock is the only budget. Whatever is left before 17:00 is what you
// can still spend today.
function hoursLeft(hour) {
    return END_HOUR - hour;
}
...
function renderStatus() {
    ...
    const remaining = hoursLeft(gameState.timeHour);
    document.getElementById("current-hours").textContent = `${remaining} SAAT`;
    ...
    // One pip per remaining hour of the shift
    const pipsContainer = document.getElementById("hour-pips");
    pipsContainer.innerHTML = "";
    for (let i = 0; i < SHIFT_HOURS; i++) {
        const pip = document.createElement("div");
        pip.className = `hour-pip ${i < remaining ? "active" : ""}`;
...
function renderPersonnel() {
    ...
    gameState.personnel.forEach(person => {
        ...
        if (isDead) {
            ...
            hintText = "Çift Tık: Tanış (1 saat)";
        }
...
function meetPerson(personId) {
    ...
    if (hoursLeft(gameState.timeHour) < MEET_HOURS) {
        alert(`Mesai bitmek üzere! Tanışmak ${MEET_HOURS} saat sürer ve gün 17:00'de kapanıyor. Günlük görevi başlatabilirsin.`);
        return;
    }

    // Advance the clock
    gameState.timeHour += MEET_HOURS;
    person.isMet = true;

    logEvent(`${person.name} ile tanışıldı. Rol: ${person.role}. (${MEET_HOURS} saat, Saat ${formatTime(gameState.timeHour)})`, "action");

    renderAll();
    openPersonModal(personId);
}

// ACTION: RUN ANOMALY INDICATOR SCAN (+2 Hours)
function runIndicatorScan(personId, indicatorIndex) {
    ...
    if (hoursLeft(gameState.timeHour) < SCAN_HOURS) {
        alert(`Mesai bitmek üzere! Bu belirteç taraması ${SCAN_HOURS} saat sürer ve gün 17:00'de kapanıyor.`);
        return;
    }

    // Advance the clock
    gameState.timeHour += SCAN_HOURS;
    ...
    logEvent(`[TEST] ${person.name} üzerinde ${testName} testi yapıldı (${SCAN_HOURS} saat, Saat ${formatTime(gameState.timeHour)}) -> Ölçüm: %${person[scanProp].prob} (${person[scanProp].certaintyType})`, "action");
...
function toggleTeamMember(personId) {
    ...
    if (!person.isMet) {
        alert(`${person.name} ile henüz tanışmadın! Sadece tanıştığın personelleri göreve gönderebilirsin. Önce bu kişiyle tanışmalısın (${MEET_HOURS} saat).`);
        return;
    }
...
function updateModalButtons() {
    ...
    if (scan1Btn) {
        if (person.isDead) {
            ...
            scan1Btn.textContent = `🔬 1. Belirteç: Biyometrik Rezonans (${SCAN_HOURS} Saat)`;
    ...
    if (scan2Btn) {
        if (person.isDead) {
            ...
            scan2Btn.textContent = `🧬 2. Belirteç: Nöro-Hücresel DNA (${SCAN_HOURS} Saat)`;
...
function nextDay() {
    ...
    logEvent(`--- GÜN ${gameState.day} BAŞLADI --- Yeni mesai başladı (09:00 - 17:00). Bugün ${gameState.dailyRequiredCount} kişi gönderilmeli!`, "system");
    renderAll();
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Indicator 1 Scan button (2 hours)
    const scan1Btn = document.getElementById("btn-scan-1");
    if (scan1Btn) {
    ...
    // Indicator 2 Scan button (2 hours)
    const scan2Btn = document.getElementById("btn-scan-2");
    if (scan2Btn) {
...
const BOT_STRATEGIES = [
    {
        ...
        tag: "Şans Odaklı",
        desc: "Gerektiğinde tanışır (1 saat), test yapmadan rastgele personelleri yollar."
    },
    {
        id: "detective",
        name: "Dedektif Bot (Çift Belirteç Taraması)",
        tag: "Derin Teşhis",
        desc: "Personelleri tanışıp 1. ve 2. belirteç testlerine sokar (2'şer saat), en düşük anomali risklileri seçer."
    },
    {
        id: "dialogue_scout",
        name: "Diyalog Ajanı (NLP İpucu)",
        tag: "İpucu Analizi",
        desc: "Tüm mesaisini tanışmaya harcar (günde 8 kişi), sözlerdeki şüpheli kelimeleri eler."
    },
    {
        id: "optimal_rotator",
        name: "Hibrit / Optimal Risk Yöneticisi",
        tag: "Gelişmiş Algoritma",
        desc: "Çift belirteçle düşük riskli insan havuzu kurar, mesai saatlerini ve risk yüzdelerini matematiksel optimize eder."
    }
];
...
function simulateSingleGame(botType) {
    ...
    for (let day = 1; day <= TOTAL_DAYS; day++) {
        let hour = START_HOUR; // the shift runs 09:00 -> 17:00
        ...
        if (botType === "random") {
            ...
            while (metRested.length < dailyQuota && hoursLeft(hour) >= MEET_HOURS) {
        ...
        // 2. DETECTIVE BOT (Meet -4, Scan 1 & Scan 2 -2 each)
            ...
            let metRested = personnel.filter(p => p.isMet && isAvailable(p));
            while (metRested.length < dailyQuota + 1 && hoursLeft(hour) >= MEET_HOURS) {
                const unmet = personnel.find(p => !p.isMet && isAvailable(p));
                if (unmet) {
                    unmet.isMet = true;
                    hour += MEET_HOURS;
            ...
            // Run Indicator 1 & Indicator 2 scans with the hours that remain
            while (hoursLeft(hour) >= SCAN_HOURS) {
                const unscanned1 = personnel.find(p => p.isMet && !p.scan1 && isAvailable(p));
                if (unscanned1 && hoursLeft(hour) >= SCAN_HOURS) {
                ...
                if (unscanned2 && hoursLeft(hour) >= SCAN_HOURS) {
        ...
        // 3. DIALOGUE SCOUT BOT (Spends the whole shift meeting people)
        else if (botType === "dialogue_scout") {
            while (hoursLeft(hour) >= MEET_HOURS) {
                const unmet = personnel.find(p => !p.isMet && isAvailable(p));
                if (unmet) {
                    unmet.isMet = true;
                    hour += MEET_HOURS;
        ...
        // 4. OPTIMAL ROTATOR BOT (Deep Risk & Fatigue Optimizer)
            ...
            let metRested = personnel.filter(p => p.isMet && isAvailable(p));
            while (metRested.length < dailyQuota + 2 && hoursLeft(hour) >= MEET_HOURS) {
                const unmet = personnel.find(p => !p.isMet && isAvailable(p));
                if (unmet) {
                    unmet.isMet = true;
                    hour += MEET_HOURS;
...
function runLiveAutoPlay(botType = "optimal_rotator") {
    ...
    function playDayStep() {
        ...
        // If not enough met & rested people for today's quota, meet unmet ones (1 hour)
        if (metRested.length < gameState.dailyRequiredCount && hoursLeft(gameState.timeHour) >= MEET_HOURS) {
